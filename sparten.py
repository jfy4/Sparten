...
class tensor:
    ...
    def conjugate(self,):
        """
        Compute the conjugate of a tensor.
        """
        return tensor((self.idx, np.conjugate(self.vals), self.shape))
    ...
    def to_array(self,):
        arr = np.zeros((prod(self.shape),))
        flat = self.reshape((prod(self.shape),))
        values = flat.vals
        indices = flat.idx[:,0]
        arr[indices] = values
